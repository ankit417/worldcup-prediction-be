...
Groups.updateGroup = (id , groupReqData , result) => {
    dbConn.query('UPDATE groups SET tournament_id=?,group_name=?,match_point=?,is_final=? WHERE id=?',[groupReqData.tournament_id,groupReqData.group_name,groupReqData.match_point,groupReqData.is_final,id],(err,res)=>{
        if(err) result(null,err)
        result(null,res)
    })
}
